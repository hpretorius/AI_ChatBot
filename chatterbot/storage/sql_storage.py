...
class SQLStorageAdapter(StorageAdapter):
    ...
    def get_latest_response(self, conversation):
        ...
        latest_statement = session.query(Statement).filter_by(
            conversation=conversation
        ).order_by(Statement.id.desc()).first()

        if latest_statement and latest_statement.in_response_to:
            latest_response_statement = session.query(Statement).filter_by(
                conversation=conversation,
                text=latest_statement.in_response_to
            ).order_by(Statement.id.desc()).first()

            if latest_response_statement:
                statement = latest_response_statement.get_statement()
            else:
