...
class ChatterBotResponseTestCase(ChatBotTestCase):
    ...
    def test_statement_added_to_recent_response_list(self):
        ...
        """
        conversation = 'test'
        statement = Statement(text='Wow!', in_response_to='Ok', conversation=conversation)
        ...
        print(self.chatbot.storage.filter()[0].serialize())

        self.assertIsNotNone(response_statement)
        self.assertEqual(statement.text, response)
        self.assertEqual(statement.in_response_to, response_statement.text)
